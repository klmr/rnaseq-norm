...
* <span class="em">But not entirely the same</span>
...
size_factors_ = function (counts, genes, group) {
    counts %>%
        group_by_(genes) %>%
...
norm_counts = counts %>% size_factors_('Gene', 'Library') %>%
...
ggplot(norm_counts, aes(Gene, Count, fill = Gene)) +
    ...
    scale_fill_manual(values = gene_colors, guide = FALSE) +
    ylab('') +
    ggtitle('Count / library size factor') +
    nogrid()
```
...
* [DESeq2 vignette](https://www.bioconductor.org/packages/devel/bioc/vignettes/DESeq/inst/doc/DESeq.pdf)
